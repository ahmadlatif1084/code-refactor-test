...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        $userType = '';$emergencyJobs = array(); $noramlJobs = array();
        if ($currentUser && $currentUser->type == 'customer') {
            ...
            $userType = 'customer';
        } elseif ($currentUser  && $currentUser->type == 'translator') {
        ...
        if ($jobs) {
            foreach ($jobs as $jobitem) {
                ($jobitem->immediate == 'yes') ? $emergencyJobs[] = $jobitem : $noramlJobs[] = $jobitem;
            }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        (isset($page)) ?  $pagenum = $page :  $pagenum = "1";
        $currentUser = User::find($user_id);
        $userType = ''; $emergencyJobs = array(); $noramlJobs = array();
        if ($currentUser && $currentUser->type == 'customer') {
        ...
        } elseif ($currentUser && $currentUser->type == 'customer') {
            $jobs_ids = Job::getTranslatorJobsHistoric($currentUser->id, 'historic', $pagenum);
